...
const DataEnrichment = (() => {
  ...
  function mergeCrew(target, source) {
    ...
    const existing = new Set(result.map(c => (c.name || '').trim().toLowerCase()));
    source.forEach(s => {
      const key = (s.name || '').trim().toLowerCase();
      if (!key) return;
      if (existing.has(key)) {
        // Fill blank fields on match
        const idx = result.findIndex(c => (c.name || '').trim().toLowerCase() === key);
        if (idx >= 0) {
  ...
  function mergeHotel(target, source) {
    ...
    source.forEach(s => {
      const gKey = (s.guest || '').trim().toLowerCase();
      const dKey = (s.checkin || '').trim();
      if (!gKey && !s.hotel) return;
      const exists = result.some(h =>
        (h.guest || '').trim().toLowerCase() === gKey && (h.checkin || '').trim() === dKey
      );
      if (!exists) {
        result.push({ ...s });
      } else {
  ...
  function addCrewToPayroll(crewList, payrollBlocks, tables, rosterData) {
    ...
    payrollBlocks.forEach(b => {
      ...
      rows.forEach(r => {
        if (r[1]) existingNames.add(r[1].trim().toLowerCase());
      });
    });
  ...
  function addCrewToPerDiems(crewList, pdBody, defaultRate) {
    ...
    const existing = new Set((pdBody || []).map(r => (r[1] || '').trim().toLowerCase()));
    let added = 0;
    crewList.forEach(c => {
      const name = (c.name || '').trim();
