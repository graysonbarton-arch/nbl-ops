// ─── HTML escaping (prevent XSS in user-provided data) ──────────────────────
function esc(str) {
  if (!str) return '';
  return String(str)
    .replace(/&/g, '&amp;')
  ...
  return base(`
    ...
    <div class="meta">
      <div class="meta-row">
        ...
        <span class="meta-value">${esc(eventName)}</span>
      </div>
      <div class="meta-row">
        <span class="meta-label">Event Date</span>
        <span class="meta-value">${esc(eventDate)}</span>
      </div>
      <div class="meta-row">
        <span class="meta-label">Venue</span>
        <span class="meta-value">${esc(venue) || '—'}</span>
      </div>
      <div class="meta-row">
        <span class="meta-label">Submitted By</span>
        <span class="meta-value">${esc(submitterName)}</span>
      </div>
      <div class="meta-row">
        <span class="meta-label">Review Due By</span>
        <span class="meta-value" style="color:#e67e22;">${esc(deadline)}</span>
      </div>
    </div>
...
export function reminder3Email({ eventName, eventDate, deadline, daysLeft, submitterName }) {
  return base(`
    ...
    <h1>${esc(daysLeft)} Days Left</h1>
    <p>This is your mid-week reminder. The post-event review for <strong style="color:#f5f5f0;">${esc(eventName)}</strong> has not been submitted yet.</p>
    ...
    <div class="meta">
      ...
      <div class="meta-row">
        ...
        <span class="meta-value" style="color:#e67e22;">${esc(daysLeft)}</span>
      </div>
    </div>
...
export function reminderFinalEmail({ eventName, eventDate, deadline, daysLeft, submitterName }) {
  return base(`
    ...
    <h1>${daysLeft === 0 ? 'Due Today' : `${esc(daysLeft)} Day${daysLeft !== 1 ? 's' : ''} Left`}</h1>

    <div class="warning">
      ⚠ This is your final reminder. Failure to submit the post-event review for <strong>${esc(eventName)}</strong> by ${esc(deadline)} will be escalated to leadership.
    </div>
    ...
    <div class="meta">
      ...
      <div class="meta-row">
        ...
        <span class="meta-value" style="color:#c0392b;">${esc(deadline)}</span>
      </div>
    </div>

    <a href="${process.env.SITE_URL || 'https://nbl-ops.vercel.app'}/?tab=postevent" class="cta" style="background:#c0392b; color:#fff;">Submit Now — ${esc(deadline)}</a>
  `);
}
...
export function overdueAlertEmail({ eventName, eventDate, deadline, submitterName, submitterEmail }) {
  return base(`
    ...
    <div class="warning">
      The post-event review for <strong>${esc(eventName)}</strong> was due ${esc(deadline)} and has not been submitted. This is an automated escalation.
    </div>
    ...
    <div class="meta">
      ...
      <div class="meta-row">
        ...
        <span class="meta-value">${esc(submitterName)} (${esc(submitterEmail) || 'no email on file'})</span>
      </div>
    </div>

    <p>Follow up directly with ${esc(submitterName)} to ensure this review is completed immediately.</p>
  `);
}
